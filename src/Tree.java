...
//Tree class that is a subclass of Plant
public class Tree extends Plant {
    ...
    // tree constructor
    public Tree(int row, int col, char id) {
    ...
    public static char[][] grow(int num, Plant myPlant) {
        for (int i = 0; i < num; i++) {
            ...
            myPlant.plantPlot[4 - result][2] = myPlant.Id;
        }
            ...
            for (int i = 0; i < myGarden.plot.length; i++) {
                for (int j = 0; j < myGarden.plot[i].length; j++) {
                    if (myGarden.plot[i][j] != null && myGarden.plot[i][j]
                            .checkPlantType(myGarden.plot[i][j].name) == 3) {
